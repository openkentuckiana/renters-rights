...
class ItemImageModelTests(TestCase):
    ...
    def test_validate_image_height(self):
        with self.assertRaisesRegexp(
            ValidationError,
            "Images must be over 50 pixels tall and wide. Please upload a larger image.",
        ):
            UnitImage.objects.create(
                image=self.get_image_file(size=(49, 500)), item=ItemImageModelTests.i
            )
    ...
    def test_validate_image_width(self):
        with self.assertRaisesRegexp(
            ...
            UnitImage.objects.create(
                image=self.get_image_file(size=(500, 49)), item=ItemImageModelTests.i
            )
    ...
    def test_validate_thumbnails_created(self):
        image = UnitImage.objects.create(
            image=self.get_image_file(size=(20, 20)), item=ItemImageModelTests.i
        )
    ...
    def test_image_downsized_if_larger_than_max_size(self):
        image = UnitImage.objects.create(
            image=self.get_image_file(size=(21, 21)), item=ItemImageModelTests.i
        )
        assert_that(default_storage.exists(image.image.path), equal_to(True))
    ...
    def test_images_are_cleaned_up_on_delete(self):
        ...
        assert_that(
            default_storage.exists(f"{filename}-10{extension}"), equal_to(False)
        )
